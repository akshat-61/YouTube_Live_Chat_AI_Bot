...
TOKEN_FILE             = "extramarks_token.json"
...
def _is_token_expired(token: str) -> bool:
    ...
    if buffer_ts >= exp_dt:
        ...
        print(f"[TokenManager] Token expires in {remaining}s — refreshing now.")
        return True

    remaining = int((exp_dt - datetime.now(timezone.utc)).total_seconds())
    print(f"[TokenManager] ✅ Token valid for {remaining}s — no refresh needed.")
    return False
...
def _save_token(token: str):
    ...
    print(f"[TokenManager] ✅ Token saved to {TOKEN_FILE}")
...
def _login() -> str:
    print("[TokenManager] Logging in to Extramarks...")
    ...
    username = _require_env("EM_USERNAME")
    print(f"[TokenManager] ✅ Login successful for '{username}'.")
    return token
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n[TokenManager] Token (first 60 chars): {token[:60]}...")
        if exp_dt:
            print(f"[TokenManager] Expires at: {exp_dt.strftime('%Y-%m-%d %H:%M:%S UTC')}")
